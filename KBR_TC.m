function [ X ] = KBR_TC(T, Omega, opts)
...
X         = initialization_M( sizeT, known, data );
...
if nargin<3
    ...
    if isfield(opts,'tol')       ; tol = opts.tol;               else tol = 1e-7;     end
    if isfield(opts,'rank')      ; rank = opts.rank;             else rank = size(T); end
